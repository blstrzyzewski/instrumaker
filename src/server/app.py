...
def get_one_track():
    ...
    item = io.BytesIO()
    drums.export(item, format="mp3", bitrate="320k")
    item.seek(0)
    response = make_response(send_file(item, mimetype="audio/mp3"))
...
def save_record():
    ...
    beat = io.BytesIO()
    z.export(beat, format="mp3", bitrate="320k")
    beat.seek(0)
    response = make_response(send_file(beat, mimetype="audio/mp3"))
    response.headers.add("Access-Control-Allow-Origin", "*")
    response.cache_control.max_age = 5
    return response
...
def combine_drums():
    ...
    temp = io.BytesIO()
    silence.export(temp, format="mp3", bitrate="320k")
    temp.seek(0)
    response = make_response(send_file(temp, mimetype="audio/mp3"))
    response.headers.add("Access-Control-Allow-Origin", "*")
    response.cache_control.max_age = 5
    return response


@app.route("/testtesting", methods=["GET"])
def testtesting():
    tempo = int(request.args["tempo"]) * 2
    response = make_response(dict(filess="sss"))
    return response
...
def getDrums():
    ...
    fx = (
        AudioEffectsChain().reverb(wet_gain=1)
        #  .overdrive(gain=99)
        # .delay(gain_out=1.0)
    ...
    sf.write(tempy, data, sr, format="mp3", bitrate="320k")
    tempy.seek(0)
    print("wwww", len(drums), len(drums.raw_data))
    # indStr=str(random.randint(1,1000))
    # dfilename='drums'+indStr+'.wav'
    # drums.export(session["dirpath"]+'\\drums\\'+dfilename, format="mp3",bitrate="320k")
    # return send_file(session["dirpath"]+'\\drums\\'+dfilename)
    return send_file(tempy, mimetype="audio/mp3")
...
def sfx(tempo):
    ...
    sfx_audio = (
        AudioSegment.from_file(sfx)
        .apply_gain(-10.0)
        .fade_in(duration=int(round(0.25 * ms_per_beat)))
        .fade_out(duration=int(round(0.25 * ms_per_beat)))
    )
    ...
    return AudioSegment.silent(duration=32 * ms_per_beat).overlay(
        sfx_audio, loop=loop_bool
    )
...
def audio_options():
    ...
    silence = AudioSegment.silent(duration=64 * ms_per_beat)
    melody = (
        silence.overlay(melody, position=0, loop=True)
        .overlay(sfx_S, position=0)
        .fade_in(duration=8 * ms_per_beat)
    )
    ...
    effects.export(file_buffer, format="mp3", bitrate="320k")
    file_buffer.seek(0)
    response = make_response(send_file(file_buffer, mimetype="audio/mp3"))
